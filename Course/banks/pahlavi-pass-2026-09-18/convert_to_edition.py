#!/usr/bin/env python3
"""Turn the Pahlavi course bank (two flat files, one per language) into the
two play-shape files a course edition ships:

    Web/courses/pahlavis/data/bank-en.js -> window.COURSE_CLUES_PAHLAVIS
    Web/courses/pahlavis/data/bank-fa.js -> window.COURSE_CLUES_PAHLAVIS_FA
...
This is the Pahlavi pass. It is the Qajar converter's shape, because the source
is the same shape: two flat files, one per language, with flat provenance
(`book_title`, `author`, `page`) and a `theme` the pass already chose. What the
Pahlavi source does differently, and what this script therefore tolerates:

  * `distractor_rationales` is a **list** of three objects (the Qajar pass wrote
    a dict keyed by the option). This converter does not read the field -- the
    engine deals its own distractors -- so either shape converts;
  * `specificity_prompt` is `""` rather than `null`, and again is not read;
  * `correct_option_index` is 0 on every row. The Qajar pass had it spread, so
    the repair branch below is kept: if a row's index ever disagrees with where
    the answer actually sits in `options`, that is repaired and reported rather
    than shipped wrong.
...
That is not a fault to be repaired: each board stays honest about itself.
...
def to_play(row, lang, buckets, report):
    ...
    out = {
        # ids mirror across the language pair, as in the archive and the other courses
        "id": row["id"],
        "round": row["round"],
        "value": row["value"],
...
def main():
    ...
    # every category should render the same subtitle in both languages
    ...
    a = emit(en, COURSE_GLOB, out["en"], "Pahlavis -- English bank")
    b = emit(fa, COURSE_GLOB + "_FA", out["fa"], "Pahlavis -- Persian bank")
    print("\nwrote %s (%d KB)" % (en, a // 1024))
    print("wrote %s (%d KB)" % (fa, b // 1024))

    # This script writes the banks itself, which is exactly how the Qajar pass
    # stepped around `Course/build_edition.sh` -- the gate that checks difficulty
    # against the rung and repeats across categories. That edition shipped with
    # 200 rows mislabelled and 10 questions asked twice because nothing ran it.
    # A converter that writes a bank now runs the gate or exits non-zero.
