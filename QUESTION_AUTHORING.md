...
the thing that learns to read it — a `COURSE_SHELVES` line per absorbed course, its groups
reproduced verbatim with their own headings and chips, and the course's file never written
to. The shelf is counted (`Tools/check_readings.py`, asserted **91 for MAIN** and 42 for
IR4595), so the count moves in the same commit and the move is stated.
