#!/usr/bin/env python3
"""
High-Speed Flawless Question Bank Builder.
Fixes everything across the 1,000 verified clues:
1. Enforces strict rising difficulty progression by price:
...
def run():
    print("==================================================")
    print("EXECUTING FLAWLESS 1,000 CLUES OVERHAUL")
    print("==================================================")

    # 1. Load English verified clues
    ...
    # 2. Load existing Persian copy
    with open("QuestionBank/persian_clues.json", "r", encoding="utf-8") as f:
        existing_fa_copy = json.load(f)
    print(f"Loaded {len(existing_fa_copy)} Persian copy entries.")

    # 3. Update difficulty for all clues based on price
    for c in en_clues:
        c['difficulty'] = get_difficulty(c['value'], c['round'])

    # 4. Build comprehensive option dictionary
    opt_map = {}
    for c in en_clues:
        cid = c['id']
    ...
    sorted_keys = sorted(opt_map.keys(), key=lambda x: -len(x))
    ...
    # High-register Persian sentence replacements
    replacements = [
    ...
    print("\nALL 1,000 CLUES FIXED AND SAVED SUCCESSFULLY! ✓")
