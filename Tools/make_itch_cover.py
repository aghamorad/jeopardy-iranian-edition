...
Same art as the banner, `Web/assets/logo-iranian-pack.png` — the front door's
lockup. The two files differ only in shape. A README is wide, so the banner is
1600x655, about 2.4:1. A cover is 1.26:1, and the lockup is 2.5:1, so the banner
dropped into the dashboard would be cropped to its middle and lose the first and
last letters of JEOPARDY!. Hence a plate built for the frame instead.

Composited on the room colour for the same reason the banner is: the lockup is
white lettering and a dark skyline on transparency, and on the light half of a
