...
/* A stalled request is shown as a failed asset, never mistaken for readiness. */
var BOOT_FAIL_MS = 30000;

function bootWarm() {
  var section = el('screen-boot'), fill = el('boot-fill'), track = el('boot-track');
  var retry = el('boot-retry'), seen = {}, list = [], done = 0, failed = [];
  var settled = false, started = Date.now(), warm = [];
  var i, j;
  ...
  for (i = 0; i < imgs.length; i++) add(imgs[i].getAttribute('src'), 'image');
  for (i = 0; i < BOOT_ART.length; i++) add(BOOT_ART[i], 'image');
  ...
  for (i = 0; i < shipped.length; i++) add(shipped[i].src, shipped[i].kind);
  ...
  var block = function () {
    if (settled) return;
    settled = true;
    if (section) section.setAttribute('data-boot', 'failed');
    var word = el('boot-status');
    if (word) word.textContent = T('boot.failed');
    if (retry) {
      retry.hidden = false;
      retry.textContent = T('boot.retry');
      retry.onclick = function () { bootWarm(); };
    }
  };

  var finish = function () {
    if (settled) return;
    if (failed.length) { block(); return; }
    settled = true;
  ...
  setTimeout(function () {
    if (!settled && done < list.length) {
      failed.push('timed out');
      block();
    }
  }, BOOT_FAIL_MS);
  for (i = 0; i < list.length; i++) {
    (function (asset, index) {
      ...
      var fired = false;
      var tick = function (bad) {
        if (fired) return;
        fired = true;
        if (bad) failed.push(asset.src);
        done++;
        paint();
      ...
      if (asset.kind === 'font') {
        ...
        face.load().then(function (loaded) {
          document.fonts.add(loaded); tick(false);
        }).catch(function () { tick(true); });
      } else if (asset.kind === 'audio') {
        var audio = document.createElement('audio');
        warm.push(audio);
        audio.preload = 'auto';
        audio.oncanplaythrough = function () { tick(false); };
        audio.onerror = function () { tick(true); };
        audio.src = asset.src;
        audio.load();
      } else {
        var im = new Image();
        warm.push(im);
        im.onload = function () { tick(false); };
        im.onerror = function () { tick(true); };
        im.src = asset.src;
      }
    })(list[i], i);
  }
