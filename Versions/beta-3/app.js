...
var Sound = (function () {
  ...
  /* Music is a single slot — switching cues fades the old one out rather
     than stacking loops. Returns `true` when the cue is already on the air,
...
function initLobby() {
  ...
  var runOpening = function () {
    ...
    S.openTimer = setTimeout(function () {
      S.openTimer = null;
      /* She plays over whatever screen the player has moved on to — the cold
         open is a cold open, not a gate. */
  ...
  var doneOpening = function () {
    ...
    if (S.screen === 'splash' || S.screen === 'lobby' || S.screen === 'setup') {
  ...
  var begin = function (lang) {
    ...
    show('lobby');
    /* If the cold open is still running it owns the music; doneOpening hands
       the theme over when she finishes. */
...
/* Four brains, one per difficulty. A brain is a set of habits rather than a
   skill score: how far into the window a robot's thumb lands, how often it is
   even paying attention when the read window closes, how much of its stack it
   will put on a wager, and how long it sits on the floor before answering.
   Nothing here reads the other contestants' scores, because a robot that plays
   the room is a different and much less amusing opponent than one that plays
   the clue. */
var Bots = (function () {
  var BRAINS = {
    easy:   { window: [3600, 7400], alert: 0.45, accuracy: 0.38,
              wager: [0.05, 0.20], think: [1400, 3000] },
    normal: { window: [2200, 5400], alert: 0.68, accuracy: 0.58,
              wager: [0.10, 0.35], think: [1000, 2200] },
    hard:   { window: [1200, 3600], alert: 0.88, accuracy: 0.78,
              wager: [0.20, 0.55], think: [700, 1600] },
    brutal: { window: [700, 2400],  alert: 1.00, accuracy: 0.92,
              wager: [0.35, 0.85], think: [400, 1100] }
  };
  ...
  /* The buzzers have opened. Each robot decides for itself whether it was
     listening and when its thumb moved; one of them landing at the far end of
     its window is the whole reason the window has a far end. */
  function armBuzzers() {
    cancel();
    ...
    S.players.forEach(function (p, i) {
      ...
      var at = b.window[0] + Math.random() * (b.window[1] - b.window[0]);
      /* Someone else may already have rung in. A late press is not a steal:
         the race is over and the room has moved on. */
  ...
  return {
    seatIsBot: seatIsBot, isBot: isBot, cancel: cancel,
