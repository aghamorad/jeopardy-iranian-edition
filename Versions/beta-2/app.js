...
function initLobby() {
  ...
  /* The house lights. The underscore comes up under the title card and stays
     there while she talks — it is a bed, not a cue, so it is never ducked. The
     moment she finishes, the title card goes live and the room hands over to the
     show's theme, which then runs the rest of the night. She will not sit
     through it twice: coming back to the title card later leaves the music
     alone. */
  ...
  /* Sound.voice always calls back — a missing cue, a refused play and a cue
     ...
     locked. The underscore has done its job by now, so this is where it hands
     over: "press any button" is live and the show's theme takes the room. */
...
function startBoardClock() {
  ...
  var paint = function () {
    paintClock(node, S.boardRemaining, BOARD_SECONDS, 'Pick a clue', 5);
  };
  paint();
...
function closeClue() {
  ...
  if (!anyUnsolved()) {
    if (S.round === 'single') {
      ...
      renderRounds();
      renderBoard();
      showRoundCard('Round Two', 'Double Jeopardy!');
      return;
    }
    startFinal();
    return;
...
function startFinal() {
  ...
  showRoundCard('The Final Round', 'Final Jeopardy!');
...
function startFinalClock(idx, banner) {
  ...
  var label = S.players[idx].name + ' · wager ' + fmtT(S.clue.value);
  var paint = function () {
...
function startMatch() {
  ...
  renderRounds();
  renderBoard();
  showRoundCard('Round One', 'Jeopardy!');
}
