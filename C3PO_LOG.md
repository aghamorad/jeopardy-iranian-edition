...
## 2026-09-21 — Khorshid Khanoom, and the 1.0.16 cut replayed

**One master, fourteen targets.** `Tools/make_app_icons.py` cuts the art to the macOS
iconset and `.icns`, the iOS asset catalog, `Web/assets/icon-512.png` and the Apple touch
icon, and every Android density including the adaptive icon's foreground. Full-bleed onto
opaque black everywhere, because a launcher or Dock tile draws its own mask and a transparent
margin comes out as a ragged rim. The adaptive foreground is the one exception: the tile is
scaled to two thirds and centred, since only that much survives any mask.

**The cutter picks its input by lexicographic order, and that bit me.** `MASTER =
sorted(GENERATED.glob("icon-main-*.png"))[-1]`. `-` is 0x2D and `.` is 0x2E, so a name ending
`-khorshid-khanoom-v2.png` sorts *before* one ending `-khorshid-khanoom.png` — the refinement
lost to the thing it refined. The cutter ran against the older art and printed the older
filename, and I read past it twice. Fixed by renaming the superseded master to `-v1`, leaving
the `-v1`/`-v2` pair as the only comparison that matters. The lesson is not "check the
print", it is that a tool selecting its input by sort needs its inputs named for the sort.

**The master now has a home in the tree.** `Icon/icon.png`, byte-identical (`ee06c5f8`) to the
master the cutter reads. `Assets/Generated/` is gitignored, so a master living only there is
one `git clean` away from gone.

**The 1.0.16 cut had to be replayed.** Commit `1e97b40` adopted the first Khorshid art and
tagged 1.0.16, and the three bundles were built against it — the refined redraw superseded
that art, so the live tree was re-cut, `Versions/v1.0.16` frozen, and all three bundles
rebuilt. Each verified by hash from inside its own container: the IPA's `icon-512.png` is
`2f6b206b` and its touch icon `b8b17193`, the macOS bundle's `AppIcon.icns` is `7fa11da9`,
and the APK's launcher is byte-identical to the tree's `b5a7317c`. The iOS app icon inside the
IPA re-encodes to CgBI, which is why PIL cannot open it — `sips` converts it, and it renders
the new art.

**The dist Android APK was stale and nothing would have caught it.** Gradle writes to
`Android/app/build/outputs/apk/release/app-release.apk`; no build path copies from there into
`dist/`. The 1.0.16 APK was copied by hand. Worth automating before the next cut.

**1.0.14 and 1.0.15 are not repainted.** They shipped with the earlier art, and
`snapshot_web.sh` is deliberately built never to overwrite. A snapshot records what shipped;
repainting one makes the release trail tidier and false. 1.0.16 is the icon release.

**Two stale numbers fixed.** `sidestore.json` carried `size: 66758013` in two places — the
pre-rebuild IPA — and now carries 67263138. The README download table said 32/33/32 MB against
artifacts of 63/64/62 MB.

**Validation.** All 12 sweep steps passed, including 27/27 Swift tests.
